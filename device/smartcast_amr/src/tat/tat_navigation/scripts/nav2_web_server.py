...
app = Flask(
    __name__,
    static_folder=BASE_DIR,   # 같�? ?�더??index.html ?�빙
    static_url_path=""
)

ros_node = None   # ?�역 ROS ?�드 ?�인??
...
class Nav2WebBridge(Node):
    def __init__(self):
        ...
        # ---- local costmap: costmap / costmap_raw ?????�도 ----
        ...
        # ---- global costmap: costmap / costmap_raw ?????�도 ----
        ...
        # Nav2 ?�션 ?�라?�언??        self.nav_client = ActionClient(self, NavigateToPose, "navigate_to_pose")
    ...
    def slam_reset(self) -> bool:
        """
        /slam_toolbox/reset ?�출: ??�??�작(?�재 pose-graph 리셋).
        """
        ...
        # 결과??굳이 기다리�? ?�고 True 반환 (비동�?
        self.get_logger().info("[WEB] Requested SLAM reset.")
        return True

    def slam_save_map(self, name: str) -> bool:
        """
        /slam_toolbox/save_map ?�출: ?�재 SLAM 맵을 pgm+yaml�??�??
        name: ?�일 ?�름 (?�장???�이). slam_toolbox가 ?�행중인 ?�렉?�리???�?�됨.
        """
...
@app.route("/")
def serve_index():
    """브라?��??�서 / ?�근?�면 index.html 반환"""
    return send_from_directory(BASE_DIR, "index.html")


@app.route("/api/state")
def api_state():
    """?�재 �?+ TF 기반 pose + 경로 + local/global costmap 반환"""
    global ros_node
    if ros_node is None:
...
def api_slam_save_map():
    """
    SLAM Toolbox /save_map ?�출: ?�에??'�??�?? 버튼 ?��?????
    body: { "name": "tat_lab_office" }
    """
    global ros_node
    if ros_node is None:
        return jsonify({"success": False, "msg": "ROS not ready"}), 500
    ...
    if not name:
        # 비어?�으�??�짜 기반 기본 ?�름
        name = time.strftime("tat_map_%Y%m%d_%H%M%S")
