...
class StateManager:
    ...
    def _seed_res_pool(self) -> None:
        ...
        # env-based 자원 disable (항상 적용).
        disabled_env = os.environ.get("MGMT_DISABLED_RES_IDS", "").strip()
        if disabled_env:
            for res_id in [s.strip() for s in disabled_env.split(",") if s.strip()]:
                if res_id in self._res_list:
                    logger.info("[StateManager] env disabled 자원 제외: %s", res_id)
                    del self._res_list[res_id]

        # ROS2 action 가용성 기반 자원 필터 (옵션, 기본 활성).
        if os.environ.get("MGMT_DYNAMIC_RES_FILTER", "1") != "0":
            self._filter_unavailable_resources()

    def _filter_unavailable_resources(self) -> None:
        """`ros2 action list` 결과로 응답 없는 자원을 _res_list 에서 제거.

        env-based MGMT_DISABLED_RES_IDS 는 _seed_res_pool 본문에서 직접 처리.
        """
        import subprocess
        import time as _time

        actions: set[str] = set()
        # 자원별 핵심 action 1개씩 보일 때까지 retry (또는 안정 시점까지 12회 × 2.5초 wait).
        # 단순 count 임계는 cast_servers(9개) 만 보이는 시점에 trigger 되어 TAT 까지 제외하는 버그.
        wanted_any_of_each = {
            "TAT": ("/TAT1/dock_robot", "/TAT2/dock_robot", "/TAT3/dock_robot"),
            "MAT": ("/pattern_pick_action",),
            "PAT": ("/pat/execute_task",),
        }
        stable_count = 0
        for attempt in range(12):
            try:
                ...
                new_actions = set(result.stdout.split())
            except Exception as exc:
                logger.warning(
                    "[StateManager] ros2 action list 실패 attempt=%d: %s", attempt + 1, exc
                )
                new_actions = set()
            # 자원 타입별 핵심 action 1개씩이라도 보이면 충분
            covered = sum(
                1 for actions_set in wanted_any_of_each.values()
                if any(a in new_actions for a in actions_set)
            )
            if new_actions == actions and len(new_actions) > 0:
                stable_count += 1
            else:
                stable_count = 0
            actions = new_actions
            if covered >= len(wanted_any_of_each) or stable_count >= 2:
                break
            _time.sleep(2.5)

        if not actions:
            logger.warning(
                "[StateManager] ros2 action list 결과 없음 — 자원 필터 skip "
                "(daemon warm-up 부족 가능)"
            )
            return

        logger.info(
            "[StateManager] ros2 action list 결과 %d개 발견 (attempt=%d)",
            len(actions), attempt + 1,
        )

        probes = {
            "TAT": "/{res_id}/dock_robot",
            "MAT": "/pattern_pick_action",     # 단일 MyCobot — res_id 무관
            "PAT": "/pat/execute_task",        # 단일 PAT
        }
        to_remove: list[str] = []
        for res_id, info in self._res_list.items():
            res_type = info.get("res_type")
            probe_template = probes.get(res_type)
            if probe_template is None:
                continue
            probe = probe_template.format(res_id=res_id)
            if probe not in actions:
                to_remove.append(res_id)

        for res_id in to_remove:
            logger.info("[StateManager] ROS2 action 미가용 자원 제외: %s", res_id)
            del self._res_list[res_id]
    ...
    def _handle_pa_gp_completion(self, task_meta: dict[str, Any]) -> bool:
        ...
        if db_complete is not None:
            if isinstance(db_complete, dict):
                ...
                logger.info(
                    "[StateManager] PA_GP completion: ord_id=%s gp_qty=%s qty=%s complete=%s",
                    ord_id,
                    gp_qty,
                    target_qty,
                    complete,
        ...
        logger.info(
            "[StateManager] PA_GP completion(memory): ord_id=%s gp_qty=%s qty=%s complete=%s",
            ord_id,
            gp_qty,
            target_qty,
            is_complete,
